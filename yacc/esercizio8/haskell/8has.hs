...
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Esp","Parentesi","let","cond","else","'='","'('","')'","'['","']'","'+'","'-'","'*'","int","identificatore","%eof"]
        bit_start = st * 19
        bit_end = (st + 1) * 19
...
happyReduction_8 (_ `HappyStk`
	...
	(HappyTerminal (TokenIdent happy_var_4)) `HappyStk`
	_ `HappyStk`
	_ `HappyStk`
	_ `HappyStk`
	happyRest)
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenSPAREN -> cont 10;
	TokenDPAREN -> cont 11;
	TokenSQuadra -> cont 12;
	TokenDQuadra -> cont 13;
	TokenSom -> cont 14;
	TokenSott -> cont 15;
	TokenMol -> cont 16;
