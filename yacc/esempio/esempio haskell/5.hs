...
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser_perna","Expr","NumCons","Inc","While","If","\"if\"","\"while\"","\"else\"","\"+=\"","'='","'('","')'","'{'","'}'","\"int\"","\"float\"","';'","int","float","identificatore","%eof"]
        bit_start = st * 24
        bit_end = (st + 1) * 24
...
happyReduction_8 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn5
		 (AsgInt happy_var_2 happy_var_4
	) `HappyStk` happyRest
...
happyReduction_9 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn5
		 (AsgFloat happy_var_2 happy_var_4
	) `HappyStk` happyRest
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenLPAREN -> cont 14;
	TokenRPAREN -> cont 15;
	TokenFwGRBracket -> cont 16;
	TokenBwGRBracket -> cont 17;
	TokenKWInt -> cont 18;
	TokenKWFloat -> cont 19;
	TokenSemiCol -> cont 20;
	TokenInt happy_dollar_dollar -> cont 21;
	TokenFloat happy_dollar_dollar -> cont 22;
	TokenIdentifier happy_dollar_dollar -> cont 23;
	_ -> happyError' ((tk:tks), [])
	}
...
parser_perna tks = happyRunIdentity happySomeParser where
